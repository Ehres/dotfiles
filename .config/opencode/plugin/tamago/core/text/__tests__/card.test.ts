...
test("age counts whole days since hatching", () => {
  assert.equal(age(0, 0, "en"), "hatched today");
  assert.equal(age(0, DAY_MS - 1, "en"), "hatched today");
  assert.equal(age(0, DAY_MS, "en"), "1 day old");
  assert.equal(age(0, 12 * DAY_MS + 5, "en"), "12 days old");
  assert.equal(age(10, 0, "en"), "hatched today", "a clock set back never goes negative");
});
...
test("progress shows the bar towards the next stage", () => {
  assert.equal(progress(tamago(career), 10, "en"), "[#---------] 2,147 / 6,000 xp → adult");
  assert.equal(progress(tamago({ ...freshCareer(0), species: "cat" }), 4, "en"), "[----] 0 / 200 xp → hatchling");
});
...
test("an elder has reached its final form", () => {
  ...
  assert.equal(progress(tamago(elder), 4, "en"), "[####] 41,467 xp · final form");
});
...
test("speciesLine hides the species while still an egg, then names it with its rarity", () => {
  assert.equal(speciesLine(tamago(freshCareer(0)), "en"), "still an egg");
  assert.equal(speciesLine(tamago({ ...freshCareer(0), species: "owl", sessions: 20 }), "en"), "owl · common"); // 200 xp: hatchling
  assert.equal(speciesLine(tamago({ ...career, species: "dragon" }), "en"), "dragon · legendary");
  assert.equal(
    speciesLine(tamago({ ...career, species: "nope" }), "en"),
    `${REFERENCE} · common`,
    "an unknown species reads as the reference",
  );
});
...
test("reveal names the species with the right article", () => {
  assert.equal(reveal("Tamago", tamago({ ...career, species: "owl" }), "en"), "Tamago hatched: an owl, common!");
  assert.equal(reveal("Momo", tamago({ ...career, species: "dragon" }), "en"), "Momo hatched: a dragon, legendary!");
});
...
test("sheetLines hides the stats while still an egg, then draws one bar per behavior Stat, aligned", () => {
  assert.deepEqual(sheetLines(tamago(freshCareer(0)), "en"), [STATS_HIDDEN.en]);
  const hatched: Career = { ...career, hatchedAt: 1789113932488, species: "cat" }; // draw: energy 5, chatter 9, sensitivity 1, patience 6
  assert.deepEqual(sheetLines(tamago(hatched), "en"), [
    "energy      [#####-----] 5",
    "chatter     [#########-] 9",
    "sensitivity [#---------] 1",
    "patience    [######----] 6",
...
test("sheetLines shows the Stats after the Modifiers of the Species", () => {
  ...
  const lines = sheetLines(tamago(dragon), "en");
