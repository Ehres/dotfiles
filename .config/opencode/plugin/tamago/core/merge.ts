...
/** Counters add up, hatchedAt is kept, the latest rename wins, the earliest Pick per Milestone wins; a Career never carries a pending `rename`. */
export function merge(career: Career, delta: Delta): Career {
  ...
  return { ...counters, hatchedAt: career.hatchedAt, picks, ...(name === undefined ? {} : { name }) };
}
